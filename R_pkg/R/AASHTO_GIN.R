...
# @purpose: Encode AASHTO Group Index Number (GIN) NASIS calculation into R pedotransfer function. Also, provides wrapper functions for dealing with L-RV-H and NASIS component SoilProfileCollections
...
ptf_AASHTO_GIN <- function(sieveno200, ll, pi, aashtocl) {
  ...
  skipll <- (!is.na(aashtocl) & aashtocl == "a-2-6") | 
            (!is.na(aashtocl) & aashtocl == "a-2-7") | 
            (sieveno200 < 35 & pi <= 10)
...
AASHTO_GIN <- function(sieveno200_l, sieveno200_r, sieveno200_h, ll_l, ll_r, ll_h, pi_l, pi_r, pi_h, aashtocl) {
  ...
  gin.l <- ptf_AASHTO_GIN(dat.l[,1], dat.l[,2], dat.l[,3], dat.l[,4])
  gin.r <- ptf_AASHTO_GIN(dat.r[,1], dat.r[,2], dat.r[,3], dat.r[,4])
  gin.h <- ptf_AASHTO_GIN(dat.h[,1], dat.h[,2], dat.h[,3], dat.h[,4])
  gin <- c(gin.l, gin.r, gin.h)
  #gin <- gin[order(gin)]
  res <- data.frame(aashind_l=gin[1], aashind_r=gin[2], aashind_h=gin[3])
  return(res)
}
